...
struct PLAYER_NAME : public Player {
	...
	*/
	...
	bool knightsPossiblePosition (Pos p) {
		Cell c = cell(p);
		return (!c.haunted && c.type != Wall && (c.id == -1 || unit(c.id).type != Witch || unit(c.id).player != 0));
	}
	...
	Dir knightsNewPosition(Pos p) {
		...
		while (!q.empty()) {
			...
			for (int i = 0; i < 8; ++i) {
				...
				if (visPos.find(pnext) == visPos.end() && knightsPossiblePosition(pnext) && pos_ok(pnext)) {
					q.push(pnext);
	...
	*/
	virtual void play () {
		iv f = farmers(0);
		for (int id : f) {
			if (unit(id).health <= 90) command(id, None);
			else command(id, farmersNewPosition(unit(id).pos));
		}
		
		iv k = knights(0);
		for (int id : k) {
			if (unit(id).health <= 90) command(id, None);
			else command(id, knightsNewPosition(unit(id).pos));
		}
		
		//iv w = witches(0);
		//for (int id : w) witchesNewPosition(unit(id).pos);
    }
};
