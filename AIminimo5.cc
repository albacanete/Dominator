...
struct PLAYER_NAME : public Player {
	...
	*/
	...
	bool witchesPossiblePosition (Pos p) {
		Cell c = cell(p);
		return (c.type != Wall && (c.id == -1 || unit(c.id).player != 0));
	}
	...
	bool minimumEnemies (Pos p) {
		...
		for (int i = 0; i < 8; ++i) {
			if (i%2 == 0) {
				if (pos_ok(p+Dir(i)) && cell(p+Dir(i)).id != -1 && unit(cell(p+Dir(i)).id).type != Witch
					&& unit(cell(p+Dir(i)).id).player != 0) ++enemies;
				if (pos_ok(p+Dir(i)+Dir(i)) && cell(p+Dir(i)+Dir(i)).id != -1 && unit(cell(p+Dir(i)+Dir(i)).id).type != Witch
					&& unit(cell(p+Dir(i)+Dir(i)).id).player != 0) ++enemies;
			}
			else {
				if (pos_ok(p+Dir(i)) && cell(p+Dir(i)).id != -1 && unit(cell(p+Dir(i)).id).type != Witch
					&& unit(cell(p+Dir(i)).id).player != 0) ++enemies;
	...
	*/
		...
		while (!q.empty()) {
			...
			for (int i = 0; i < 8; ++i) {
				...
				if (visPos.find(pnext) == visPos.end()) {
					if (knightsPossiblePosition(pnext) && (cell(pnext).id == -1  || (cell(pnext).id != -1
						&& unit(cell(pnext).id).type != Witch))) {
						q.push(pnext);
	...
	Dir witchesNewPosition (Pos p) {
		...
		while (!q.empty()) {
			Pos paux = q.front();
			if (cell(paux).id != -1 && paux != p && minimumEnemies(paux)) {
				Pos preturn = paux;
				while (visPos[preturn] != p) preturn = visPos[preturn];
				if (preturn+Bottom == p) return Top;
				if (preturn+Right == p) return Left;
			...
			for (int i = 0; i < 4; ++i) {
				...
				if (visPos.find(pnext) == visPos.end()&& (cell(pnext).id == -1  || (cell(pnext).id != -1
					&& unit(cell(pnext).id).type != Witch)) && witchesPossiblePosition(pnext)) {
					q.push(pnext);
					visPos.insert(make_pair(pnext, paux));
				}
			}
