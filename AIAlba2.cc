...
struct PLAYER_NAME : public Player {
	...
	*/
	...
	bool safePositionToAttack (Pos p, int id) {
		bool safe = true;
		if (cell(p).haunted) return false;
	...
	Dir farmersNewPosition(Pos p) {
		...
		while(!q.empty()) {
			...
			if (visPos.find(paux+Bottom) == visPos.end()) {
				if (farmersPossiblePosition(paux+Bottom) && safePositionToGo(paux+Bottom)) {
					q.push(paux+Bottom);
					visPos.insert(make_pair(paux+Bottom, paux));
				}
			}
			if (visPos.find(paux+Right) == visPos.end()) {
				if (farmersPossiblePosition(paux+Right) && safePositionToGo(paux+Right)) {
					q.push(paux+Right);
					visPos.insert(make_pair(paux+Right, paux));
				}
			}
			if (visPos.find(paux+Left) == visPos.end()) {
				if (farmersPossiblePosition(paux+Left) && safePositionToGo(paux+Left)) {
					q.push(paux+Left);
					visPos.insert(make_pair(paux+Left, paux));
				}
			}
			if (visPos.find(paux+Top) == visPos.end()) {
				if (farmersPossiblePosition(paux+Top) && safePositionToGo(paux+Top)) {
					q.push(paux+Top);
	...
	*/
		...
		while (!q.empty()) {
			...
			for (int i = 0; i < 8; ++i) {
				...
				if (visPos.find(pnext) == visPos.end()) {
					if (knightsPossiblePosition(pnext) && safePositionToGo(pnext)) {
						q.push(pnext);
	...
	Dir witchesNewPosition (Pos p, int id) {
		queue<Pos> q;
		ppMap visPos;
		vector<Dir> directions = {Bottom, Right, Left, Top};
		visPos.insert(make_pair(p, p));	
		q.push(p);
		while (!q.empty()) {
			Pos paux = q.front();
	...
	*/
	virtual void play () { 
		...
		iv w = witches(0);
		for (int id : w) {
			command(id, witchesNewPosition(unit(id).pos, id));
		}
    }
};
