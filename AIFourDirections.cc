...
struct PLAYER_NAME : public Player {
	...
	*/
	...
	bool knightsPossiblePosition (Pos p) {
		Cell c = cell(p);
		return (!c.haunted && c.type != Wall && (c.id == -1 || (c.id != -1 && unit(c.id).player != 0)));
	}
	
	bool witchesPossiblePosition (Pos p) {
		Cell c = cell(p);
		return (c.type != Wall && (c.id == -1 || (c.id != -1 && unit(c.id).player != 0)));
	}
	...
	*/
	...
	Dir witchesNewPosition (Pos p) {
		queue<Pos> q;
		ppMap visPos;
		vector<Dir> directions(4);
		if (round() <= 50) directions = {Bottom, Right, Left, Top};
		else if (round() <= 100) directions = {Right, Top, Left, Bottom};
		else if (round() <= 150) directions = {Top, Left, Bottom, Right};
		else directions = {Bottom, Top,Left, Right};
		visPos.insert(make_pair(p, p));	
		q.push(p);
		while (!q.empty()) {
			Pos paux = q.front();
