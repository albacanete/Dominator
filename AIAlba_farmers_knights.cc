 ...
 */
#define PLAYER_NAME AlbaFarmersK
...
struct PLAYER_NAME : public Player {
	...
	Dir knightsNewPosition(Pos p) {
		...
		while (!q.empty()) {
			...
			for (int i = 0; i < 8; ++i) {
				...
				if (visPos.find(pnext) == visPos.end() && knightsPossiblePosition(pnext)) {
					q.push(pnext);
	...
	*/
	virtual void play () {
		iv f = farmers(0);
		for (int id : f) {
			if (unit(id).health > 90) command(id, farmersNewPosition(unit(id).pos));
			else command(id, None);
		}
		
		iv k = knights(0);
		for (int id : k) {
			if (unit(id).health > 90) command(id, knightsNewPosition(unit(id).pos));
			else command(id, None);
		}
